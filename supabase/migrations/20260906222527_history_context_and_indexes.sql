create index commands_project on planning.commands(project_id);
alter table planning.history add column context jsonb not null default '{}'::jsonb;
create or replace function public.project_history(project_id uuid) returns jsonb
language sql stable security invoker set search_path='' as $$
 select coalesce(jsonb_agg(jsonb_build_object('revision',h.revision,'action',h.action,'at',h.at,'before',h.before_item,'after',h.after_item,'context',h.context) order by h.revision desc),'[]'::jsonb) from planning.history h where h.project_id=project_history.project_id;
$$;
...
begin
 ...
 if kind in ('create_project','rename_project') then
  after_state:=jsonb_build_object('title',p.name,'body',p.description);
