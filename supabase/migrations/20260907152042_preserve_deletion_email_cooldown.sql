-- Cancellation invalidates the link without erasing the send cooldown.
create or replace function account_private.deletion(action text, request_id uuid default null, signature text default null) returns jsonb
...
begin
 ...
 if action='cancel' then
  update account_private.deletion_requests set expires_at=0 where user_id=actor;
  return '{}'::jsonb;
 end if;
